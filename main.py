...
def mtime(f: callable, *a) -> float:
    t = time.perf_counter()
    for _ in range(100):
        f(*a)
    return (time.perf_counter() - t) / 100

def mmem(f: callable, *a) -> int:
    tracemalloc.start()
    f(*a)
...
def ident(rc: list) -> list:
    return rc

def rq(qf: callable, pr: tuple, pp: list):
    for x, y in pp:
        qf(pr, x, y)

# print(rc, pp)
def coll(pfn: callable, qf: callable, rc: list, pp: list) -> tuple:
    d = pfn(rc)
    pt = mtime(pfn, rc)
    pm = mmem(pfn, rc)
    # print(pt, pm)
...
def run(nm: str, pfn: callable, qf: callable, sc: list):
    print("\n=== %s ===" % nm)
    res = []
    for lb, rc, pp in sc:
